...
def train(args):
    ...
    ## Optimizer 설정하기
    optimG = torch.optim.Adam(netG.parameters(), betas=(0.5, 0.999), lr=lr)
    optimD = torch.optim.Adam(netD.parameters(), betas=(0.5, 0.999), lr=lr)
    ...
    if mode == 'train':
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
            ...
            for batch, data in enumerate(loader_train, 1):
                ...
                input = torch.randn(label.shape[0], 100, 1, 1).to(device)   # Noise Image
                output = netG(input)    
            # backward pass (netD)
...
def test(args):
    ...
    if mode == 'test':
        ...
        net, netG, netD, optimG, optimD, st_epoch = load(ckpt_dir=ckpt_dir, netG=netG, netD=netD, optimG=optimG, optimD=optimD)
